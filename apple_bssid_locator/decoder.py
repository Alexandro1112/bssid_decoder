...
def main():
	...
	if lat != "-180.0" or lon != "-180.0":
		...
		folium.Marker(location=[lat, lon], popup=geopy.Nominatim(user_agent='bssid').geocode(f'{lat}, {lon}')).add_to(map)
		map.save(f'{geopy.Nominatim(user_agent="bssid").geocode(f"{lat}, {lon}")}.html')
